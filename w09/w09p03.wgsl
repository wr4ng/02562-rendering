...
fn get_camera_ray(ipcoords: vec2f) -> Ray {
	// Set low t_max since scene is very small
	const t_max = 2;
...
fn lambertian(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, t: ptr<function, u32>) -> vec3f {
	...
	let directional_shadow = select(0.4, 0,intersect_scene(&shadow_ray, &shadow_hit));

	var V = !intersect_scene(&shadow_ray, & shadow_hit);

	// Re-add direct lighting
...
fn fresnel_R(costhetai: f32, costhetat: f32, ior_i_over_t: f32) -> f32 {
	let costhetat2 = 1 - (ior_i_over_t * ior_i_over_t) * (1.0 - costhetai * costhetai);
	let total_internal_reflection = costhetat2 < 0.0;
...
fn holdout(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, t: ptr<function, u32>) -> vec3f {
	// Environment holdout
	let random_dir = sample_cosine_weighted_direction(hit.normal, t);
	var shadow_ray = Ray(hit.position, random_dir, 0.001, 1e16);
	var shadow_hit = default_hitinfo();
	let environment_shadow = select(0.6, 0,intersect_scene(&shadow_ray, &shadow_hit));

	// Directional light holdout
	let light = sample_directional_light(hit.position);
	shadow_ray = Ray(hit.position, light.w_i, 0.001, light.dist - 0.001);
	shadow_hit = default_hitinfo();
	let directional_shadow = select(0.4, 0,intersect_scene(&shadow_ray, &shadow_hit));

	//TODO: More correct sampling of environment map (multiple samples in random directions)
	return sample_bg_texture(r.direction).rgb * (environment_shadow + directional_shadow);
}


fn intersect_scene(r: ptr<function, Ray>, hit: ptr<function, HitInfo>) -> bool {
	// Holdout plane
	if (intersect_plane(*r, hit, vec3f(0.0, 0.025, 0.0), vec3f(0.0, 1.0, 0.0))) {
		hit.shader = 7u;
		r.tmax = hit.distance;
	}
	// Bunny
...
fn sample_directional_light(pos: vec3f) -> Light {
	var light = Light();
	let dir = normalize(vec3f(1.5, -1.0, -0.3));
	const intensity = vec3f(0.5) * 3.14;
	light.w_i = - dir;
	light.L_i = intensity;
	light.dist = 1.0e32;
	return light;
...
fn sample_bg_texture(dir: vec3f) -> vec4f {
	let u = 0.5 + 1/(2*3.14) * atan2(dir.x, -dir.z);
	let v = 1/(3.14) * acos(-dir.y);
	let texColor = textureLoad(bg_texture, vec2u(vec2f(u * 4000, v * 2000)), 0);
	// let mappedColor = texColor.rgb * pow(2.0, 255 * texColor.a - 128); // Decode RGBE
	// Use texColor directly since we use non-HDR background
	return texColor;
}
...
fn main_fs(@builtin(position) fragcoord: vec4f, @location(0) coords: vec2f) -> FSOut {
	...
	var r = get_camera_ray(uv + prog_jitter);
	...
	for (var i = 0; i < max_depth; i++) {
		if (intersect_scene(&r, &hit)) {
			...
			let bgcolor = sample_bg_texture(r.direction);
