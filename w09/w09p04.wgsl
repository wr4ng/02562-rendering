...
fn holdout(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, t: ptr<function, u32>) -> vec3f {
	...
	for (var i = 0; i < M; i++) {
		let dir = sample_cosine_weighted_direction(hit.normal, t);
		let Lenv = sample_bg_texture(dir).rgb;
		total_light += Lenv;
	}
	total_light = total_light / f32(M);

	let Lenv_sampled = sample_bg_texture(r.direction).rgb;
	let Lenv_random = sample_bg_texture(random_dir).rgb;
	return Lenv_sampled * (Lenv_random / total_light);
}


fn intersect_scene(r: ptr<function, Ray>, hit: ptr<function, HitInfo>) -> bool {
	// Holdout plane
	if (intersect_plane(*r, hit, vec3f(0.0, 0.03, 0.0), vec3f(0.0, 1.0, 0.0))) {
		hit.shader = 7u;
		r.tmax = hit.distance;
	}
...
fn sample_directional_light(pos: vec3f) -> Light {
	var light = Light();
	let dir = normalize(vec3f(-1.0, -1.6, 1.0));
...
fn sample_bg_texture(dir: vec3f) -> vec4f {
	...
	let texColor = textureLoad(bg_texture, vec2u(vec2f(u * 4096, v * 2048)), 0);
	let mappedColor = texColor.rgb * pow(2.0, 255 * texColor.a - 128); // Decode RGBE
	return vec4f(mappedColor, 1.0);
}
